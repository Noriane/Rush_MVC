...
class AccueilModel extends AModel
{
    ...
    {
        $sql= "SELECT articles.id, title, content, users.username, categories.name as 'cat', tag_id, CONCAT('/Webroot/Img/',url_img), articles.create_date, articles.edit_date FROM articles INNER JOIN users ON articles.author_id = users.id INNER JOIN categories ON articles.categories_id = categories.id ORDER BY articles.id DESC LIMIT 10";
